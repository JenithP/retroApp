...
const STEPS = {
  ...
  material: {
    ask: "그 빨간 것을 부르려면 먼저 무엇이 필요하다고 할까? (누가 가져올 물건 하나)",
    pass: "누가 손에 쥘 수 있는 마른 막대를 가리켜 말한다. 「마른 막대를 가져와」처럼 그냥 막대라고 해도 통과하고, "
        + "「밟으면 딱 소리가 나는 것」, 「가볍고 잘 부러지는 것」처럼 감각이나 쓰임으로 말해도 통과한다. "
        + "막대를 가리키면서 뒤에 다른 말(비빈다, 분다 따위)이 붙어 있어도 통과시킨다.",
    fail: "나무·목재·재료·가연물 같은 범주 이름으로 말하거나, 불·번개 같은 이름을 쓴다. "
        + "다만 앞에서 함께 지은 이름(하늘 뱀의 빨간 것 따위)은 괜찮다.",
  },
  method: {
    ask: "막대를 가지고 어떻게 해야 그 빨간 것이 생긴다고 할까? (손으로 하는 동작)",
