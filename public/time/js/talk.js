...
export class Talk {
  ...
  async ask(who, text, { prefill = "", placeholder = "누에게 할 말을 쓰십시오", button = "누에게 말하기", extra = null } = {}) {
    await this._type(who, text);
    this._click = null;
    return new Promise(res => {
      this.choicesEl.innerHTML = `<form class="say">
        <textarea maxlength="300" rows="2" aria-label="누에게 할 말"></textarea>
        <div class="sayrow"><small class="cnt"></small><span class="sayact"><button type="button" class="tool extra" hidden></button><button type="submit"><span class="lbl"></span> <kbd>Enter</kbd></button></span></div>
      </form>`;
      ...
      if (extra) {                               // 곁 단추(예: 한 번 더 듣기) — 누르면 null 을 돌려준다
        const x = form.querySelector(".extra");
        x.hidden = false; x.textContent = extra;
        x.addEventListener("click", () => { this.choicesEl.innerHTML = ""; res(null); });
      }
