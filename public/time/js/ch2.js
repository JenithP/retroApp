// 2장 — 손으로 베끼던 시대.
//
// 편지는 한 부뿐이다. 조종하는 한 사람이 처음부터 끝까지 따라 쓰고, 숲길에서 후드 쓴 자를 만난다.
...
const ABBOT = "수도원장", HOOD = "후드 쓴 자", GATE = "옆 수도원 문지기";
...
export async function chapter2(G) {
  ...
  G.missLabel = "빼앗긴 편지";
  G.enterMedieval();
  G.objective("수도원 도서실. 수도원장의 부탁을 들으십시오.");
  await talk.say("", "(활자의 빛이 걷히자 돌벽과 촛불이 보였다. 양피지와 잉크 냄새가 났다.)");
  await talk.say(ABBOT, "잘 왔다, 낯선 이여. 옆 수도원에 이 편지가 닿아야 한다. 겨울을 나려면 양피지가 필요하거든.");
  await talk.say(ABBOT, "원본은 이 도서실에 두어야 한다. 네 손으로 한 부 베껴서 가져가거라. 글자가 흐트러지면 저쪽에서 읽지 못한다.");
  ...
  for (let attempt = 1; ; attempt++) {
    ...
    await talk.say(ABBOT, attempt === 1
      ? "숲길을 따라가면 옆 수도원 문이 나온다. 조심하거라. 요즘 숲에 수상한 자가 드나든다더구나."
    ...
    // 나무 사이에서 서 있다가, 우리를 보자 주먹을 휘두르며 길을 막는다
    ...
    talk.close();
    G.closeUp(null);
    await talk.say("", "(편지는 한 부뿐이었다. 수도원으로 돌아가 처음부터 다시 베껴야 한다.)");
    talk.close();
    G.backToAbbey();
  }
  ...
  talk.close();
  G.closeUp(null);
  G.walkTo(hood, [hood.pos.clone().add(G.aside)], 1.6);
  G.showGate();
  G.objective("옆 수도원 문에 편지를 전하십시오.");
  await G.interact({ pos: G.gatePos }, "편지 전하기", 2.8);
  audio.chime();
  await talk.say(GATE, "먼 길 오셨소. 한 부를 베끼는 데 꽤 걸렸겠구려. …우리도 답장을 베껴 보내려면 한참이겠군.");
