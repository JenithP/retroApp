...
export const BROKER = {
  knock: job => [
    "실례하오. 앱시장에서 왔소이다.",
    `${job.app} 앱의 ${job.screen} 화면이오. 쓴 사람들이 이럽디다 — 「${job.say}」`,
    "쓸 만하게 고쳐 주시오. 시장에 내놓을 만하면 값은 후하게 쳐드리리다.",
  ],

  next: job =>
    `또 하나 물어 왔소. ${job.app} 앱의 ${job.screen} 화면이오. ` +
    `「${job.say}」 하더이다.`,
  waiting:
  ...
  reject: v =>
    `이래서는 못 내놓소. 테스트 사용자가 시킨 일을 끝내질 못했소 — ` +
    `${v.of}가지 가운데 ${v.right}가지뿐이오. 더 고쳐서 오시오.`,
...
export const NORMAN = {
  ...
  verdict(v) {
    if (v.clean)
      return `막힘이 없었네. 이만하면 내놓아도 되겠어. 아래 단추를 누르게.`;
