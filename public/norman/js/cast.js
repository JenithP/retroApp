...
export const NORMAN = {
  ...
  lowPoint: p =>
    ...
    `내가 내는 문제를 맞히면 한 문제에 40포인트를 드립니다. 액자를 눌러 보세요.`,

  /** 판정을 보고 던지는 한마디 — 비어 있던 자리를 짚는다 */
  verdict(v) {
    if (v.clean)
      return `사용자가 막히지 않았습니다. 이 정도면 시장에 내놓아도 됩니다. 아래 버튼을 누르세요.`;
    if (v.evalGap > 0 && v.exec > 0)
      return `두 가지가 모두 막혔습니다. 어디를 눌러야 하는지도 몰랐고, 누른 뒤 결과도 알 수 없었습니다.`;
    if (v.evalGap > 0)
      return `사용자가 같은 곳을 다시 누른 이유는 결과가 보이지 않았기 때문입니다. ` +
             `조작 후 피드백을 붙여 보세요.`;
    if (v.wrong > 0)
      return `여러 번 눌렀지만 입력된 값이 의뢰와 다릅니다. 사용자가 상태를 읽기 어렵습니다.`;
    return `아직 어디를 눌러야 할지 모릅니다. 처음부터 보이는 단서를 더 붙여 보세요.`;
  },
};
