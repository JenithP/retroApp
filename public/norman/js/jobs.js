...
export const JOBS = [
  ...
  J("shop-card", "쇼핑", "겨울 아우터",
    ...
    [{ id: "c1", kind: "card", visual: "product", label: "울 더블 코트",
     ...
     { id: "c2", kind: "card", visual: "product", label: "패딩 하프 점퍼",
       ...
       stuck: { push: "사진과 상품명, 가격은 잘 보입니다. 다만 카드 전체를 눌러 " +
         "상세로 들어갈 수 있는지, 사진만 눌리는지 알기 어렵습니다." } }],
    [{ part: "c2", do: "press" }],
  ...
  J("taxi-start", "택시", "출발지",
    "출발지를 정해 보세요.",
    // 「지금 위치」는 눌러도 아무 일이 없는 미끼인데, 맡은 일이 「지금
    // 위치로 정해 보세요」였다. 글월을 그대로 따른 조가 미끼에 단서를
    // 붙이고 거짓 단서로 값을 깎였다. 이름을 갈라 놓는다.
    [{ id: "here", kind: "icon", label: "지도 가운데로", decoy: true },
     { id: "pick", kind: "button", label: "출발지 정하기",
       stuck: { name: "아래쪽 넓은 띠와 왼쪽 아래 작은 단추가 똑같이 " +
         "생겼습니다. 둘 중 어느 것이 출발지를 정하는 것인지 알 수 없습니다." } }],
    [{ part: "pick", do: "press" }],
  ...
  J("health-dot", "건강", "걸음 수",
    ...
    [{ id: "chart", kind: "card", visual: "chart", label: "요일별 걸음 수",
       ...
       stuck: { push: "요일별 막대는 잘 보입니다. 다만 막대 하나를 눌러 " +
  ...
  J("story-tap", "스토리", "여행 기록",
    ...
    [{ id: "story", kind: "card", visual: "post", label: "부산 바닷길",
       ...
       stuck: { push: "사진은 꽉 차게 보입니다. 다만 좌우를 눌러 넘기는 것인지, " +
